...
out_list = rep(rep(c("out1_scaled_index", "out3_scaled_index", "out5_scaled_index"), each = 6), 4)

treatment_list = rep(c("US-China Competition (T1)", 
                       "Sanction by US (T2)", 
                       "Gov use of data (T3)"), 24)

interaction_list = c(rep (rep (c("Nationalism (equal or above mean)", 
                                 "Nationalism (below mean)"), each = 3), 3),
                     
                     rep (rep (c("Patriotism (equal or above mean)", 
                                 "Patriotism (below mean)"), each = 3), 3),
                     
                     rep (rep (c("Conservativeness (equal or above mean)", 
                                 "Conservativeness (below mean)"), each = 3), 3),
                     
                     rep (rep (c("Antimarket (equal or above mean)", 
                                 "Antimarket (below mean)"), each = 3), 3))


position_list =           rep(c(27.5, 27,
                                17.5, 17,
                                7.5, 7,
                                25.5, 25,
                                15.5, 15,
...
position_df = data.frame ("model" = out_list, 
                          ...
                          "interaction" = interaction_list)   #a df with position

model_plot <- merge (all_model, position_df, by = c("model", "interaction", "treatment"))  #merge two tables, now results have position
...
model_plot %>%
  ...
  scale_shape_manual (values = c(16,1,15,0,17,2, 8, 5)) +
  ...
  scale_x_continuous(breaks = c(25, 15, 5),
